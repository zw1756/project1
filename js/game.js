...
Game.prototype.removes = function (i, t) {
	...
	return $.Deferred(function (remove) {
		...
		that.blockArea.splice(index, 1)
		$('.block[top=' + t + ']').eq(i).animate({'opacity': '0'}, 100).animate({'opacity': '1'}, 100).animate({'opacity': '0'}, 100).animate({'opacity': '1'}, 100).animate({
...
Game.prototype.moveLeft = function () {
	...
	if (this.pause == false) {
		...
		Promise.all([pL0, pL1, pL2, pL3]).then(function () {
			that.updateMap()
			$('.block[move=true]').each(function (i) {
				$(this).css({'left': that.map[that.r1][that.r2][i][0] + 'px'})
			})
		}).catch(function () {
			that.x += 30
		})
...
Game.prototype.rotates = function () {
	...
	if (this.pause == false) {
		...
		Promise.all([p0, p1, p2, p3]).then(function () {
			$('.block[move=true]').each(function (i) {
				$(this).css({'top': that.map[that.r1][that.r2][i][1] + 'px','left': that.map[that.r1][that.r2][i][0] + 'px'})
			})
		}).catch(function () {
			that.r2 += 1
			if (that.r2 > 3) {
...
Game.prototype.startGame = function () {
	...
	this.getRandom();
	function go() {
		that.createBlock();that.getRandom();that.yulan();that.controller();var isDown = that.autoMove();
